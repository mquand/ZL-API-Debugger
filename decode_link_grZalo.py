...
try:
    ...
    browser_builder = lambda: uc.Chrome()
except Exception:
...
class ZaloGroupGUI(QtWidgets.QWidget):
    ...
    def extract_params_from_performance(self, timeout=6):
        ...
        Trả về tham số params (URL-decoded) hoặc None.
        """
        if not self.driver:
            return None
        ...
        while time.time() < end:
            try:
                ...
                for entry in logs:
                    ...
                    url = req.get("url", "")
                    if "/api/group/link/ginfo" in url and url not in seen:
                        seen.add(url)
                        # parse query
                        try:
    ...
    def on_check_group(self):
        ...
        try:
            # --- Encode payload trước khi gửi ---
            self.client._state._cookies = self.session_cookies
            self.client._state._config["secret_key"] = self.secret_key
            payload_obj = {"link": str(cleaned_link), "clientLang": "vi"}
            encoded_params = self.client._encode(payload_obj)

            # --- Gọi ginfo endpoint ---
            url = "https://tt-group-wpa.chat.zalo.me/api/group/link/ginfo"
            params = {"zpw_type": 30, "zpw_ver": 669, "params": encoded_params}
            headers = {
                "User-Agent": self.user_agent,
                "Accept": "application/json, text/plain, */*",
                "Referer": "https://chat.zalo.me/",
            ...
            try:
                ...
                try:
                    ...
                    payload = dec[:-pad].decode(errors="ignore")
                    decoded = json.loads(payload)
                    self.log("Fallback AES decode thành công.")
                except Exception as e:
                    self.log("Decode thất bại:", e)
                    return
